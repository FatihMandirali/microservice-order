...
public class CourseService:ICourseService
{
    ...
    {
        var course = await _courseCollection.Find(course => true).ToListAsync();
        if (course.Count > 0)
        {
            foreach (var item in course)
            {
                item.Category = await _categoryCollection.Find<Category>(x => x.Id == item.CategoryId)
                    .FirstOrDefaultAsync();
            }
        }
        else
    ...
    {
        var cource = await _courseCollection.Find<Course>(x => x.Id == id).FirstOrDefaultAsync();
        if (cource is null)
        {
            return Shared.Dtos.Response<CourseDto>.Fail("Not found", 404);
        }

        cource.Category =
            await _categoryCollection.Find<Category>(x => x.Id == cource.CategoryId).FirstOrDefaultAsync();

        return Shared.Dtos.Response<CourseDto>.Success(_mapper.Map<CourseDto>(cource), 200);
    }
